...
1. 从 GitHub Releases 下载最新版 `Millwright Setup x.x.x.exe`
2. 双击运行安装程序，按提示完成安装
3. 安装完成后桌面会出现 Millwright 图标
...
2. 点击「刷新」按钮重试
3. 如果仍未检测到，尝试以管理员身份运行 Millwright
...
- Millwright 不会上传你的 CAD 文件到任何服务器
- 发送给 AI 的只有你的文字描述，不包含模型数据
- API Key 加密存储在本地，不会明文传输到 Millwright 的服务器（我们没有服务器）
