...
```
你：  做一块 80 × 50 × 10 的板，中间挖一个 ⌀10 的通孔。
...
**AI 后端由你决定。** Claude、GPT、DeepSeek、Kimi、MiniMax、Qwen、GLM，或本地 Ollama —— 任何支持 Anthropic 或 OpenAI 兼容协议的模型都行。代码开源，你只需支付自己的 API 费用。

|         | 常见 CAD AI SaaS       | **Millwright**             |
| ------- | -------------------- | --------------------------- |
| 自动化方式   | 提示词 → 一次性宏，需人工粘贴运行   | **可自我纠正的 agent 工具循环**       |
| AI 后端   | 按套餐固定                | **任你选择（自带 API Key）**        |
| 价格      | 按坐席订阅                | **免费 —— 开源，只付自己的 API 费用**   |
...
- 📦 **零外部依赖安装。** installer 内置 Python 运行时、`pywin32` 和 `pillow`，装完即用。
...
Millwright 在运行时探测当前机器可用的最佳 API，而不是假设固定版本，能自动处理不同的参数个数、缺失的 COM 成员、多个重载候选、以及不同的特征方向。一套运行时，无需针对版本单独打包。

这不是纸面上的设计，而是针对真机反馈持续加固的结果。具体来说：`cut_extrude` 会在运行时搜索参数个数和签名变体，直到成功为止，自动收敛到当前机器 SolidWorks 版本所需要的形式；特征树遍历在 COM 解析不到链表式 API 时改走 `IFeatureManager.GetFeatures`；切除方向按正向 → 反向 → 双向对称依次试探。每一条都来自真实安装环境上的真实报错。
...
- **确认门。** 拉伸切除、删除特征、压缩零部件、覆盖导出等破坏性操作，会展示工具名和完整参数，需要明确点击才会执行。
...
- **多模态主模型**（Claude、GPT、MiniMax M3、Qwen-VL 等）：截图直接进模型上下文，不用额外一步。
...
cd Millwright
npm install
pip install pywin32 pillow   # installer 内置的边车运行时，源码模式需自行安装
npm run dev
```
...
AI: start_sketch(front) → sketch_rectangle(50,30) → extrude(20)  ✓  零件已创建
...
- 🧪 **真实 SolidWorks 环境的测试报告。** Millwright 绝大部分加固都来自这里。目前还有几个多参 API（`revolve`、`fillet_edges`、`shell`、阵列与镜像家族、`create_reference_point`、`export_stl`）在等真机的宏录制器核对，记录在[待核验清单](https://github.com/raylanlin/Millwright/blob/master/docs/VERIFY-ISSUES.md)里——在你的机器上核对其中一项，就是最有价值的贡献。
...
> Millwright 还很年轻、迭代很快——目前大多数版本都是在修复真实 SolidWorks 安装环境反馈的 COM 绑定与跨版本问题，详见 [CHANGELOG.md](https://github.com/raylanlin/Millwright/blob/master/CHANGELOG.md)。
