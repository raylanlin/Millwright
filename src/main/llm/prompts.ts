...
## 圆角/倒角一律用特征，不要画进草图
\`fillet_edges\` 失败时**不要退回「把圆角画进草图轮廓」** —— 那样出来的零件特征树里只有一个拉伸，圆角和孔全焊死在草图里：改 R10 得重画整张草图，而且圆弧接不上会留下豁口（实际发生过：四个角只有两个是真圆角，另两个是小缺口）。正确做法是报告 \`fillet_edges\` 的失败原文，让用户知道是工具的问题。

同理，孔用 \`cut_extrude\` 切，不要画进外轮廓草图。**特征树是这个零件之后还能不能改的唯一依据。**
