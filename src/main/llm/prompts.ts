...
- **等截面外轮廓的圆角优先画进草图**：矩形板四角倒 R，用 \`sketch_fillet\` 在草图里做完再拉伸，一个特征成型；草图圆角有尺寸，照样可改。\`fillet_edges\` 留给真正的三维边（已成型实体的顶面边、交线等）。
...
- ⚠️ **\`circular\` 会选中整个文档里所有圆形边**（孔的圆边、圆柱的顶/底边、已有圆角的边全算）——复杂零件里整批倒容易失败。想倒"某个特征自己的边"（如圆柱顶面边），直接说清特征：工具会优先按最近特征选边。也可以请用户在 SolidWorks 里手动选中目标边，然后用 \`edges="selected"\`（工具会校验选中的确实是边，混有面/特征会拒绝）。
